...
{  
  ...
  {
    // アニメーション中の処理を記述
    float fCircleSize = getCircleSize( secondCurrent / 60.0, 50, 400);
    circle(width/2, height/2, fCircleSize);
...
// fValue: 0~1
float getCircleSize( float fValue, float fMin, float fMax )
{
  float fRad = radians( 360 * fValue );
  float fSize = fMin + ( fMax - fMin ) * (cos(fRad) + 1) / 2.0;
  return fSize;
}
