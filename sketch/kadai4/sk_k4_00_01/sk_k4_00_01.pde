/**
教会の堂内にあるステンドグラスをモチーフに，ステンドグラス模様の再帰的な表現を試みた．
特に私の地元にある教会のステンドグラスには，模様の中に窓の形状が内包されている．
このことから，再帰的に文様を生成できると考えた．
また，ステンドグラスを透過する光とガラス間を区切る枠のコントラストを表現するため，
fill関数とstroke関数を分けるようプログラムを工夫した．
 */
