...
{  
  ...
  // デ情くんスケール更新
  fDejouScale = 0.4 + 0.7 * sin( radians( 180 * deltaTimePerSec ) );
  
  // デ情くん描画
  drawDejoukun( 200, fDejouY, fDejouScale );
  ...
  {
    // アニメーション中の処理を記述
    if( fDejouTimer < 3 )  // ジャンプ
    {
      fDejouY = 200 - 100 * sin( radians( fDejouTimer * 60 ) );
      fDejouTimer += deltaTime;
    }
    else
    {
...
// デ情くん描画関数
void drawDejoukun( int iX, float fY, float fScaleX )
{
  pushMatrix();
